...
const Renderer = (() => {
    ...
    const getElementBadge = (element) => {
        if (!element) return `<span class="text-xs font-bold text-slate-400 uppercase tracking-wider">${t('misc.global', 'Global')}</span>`;
        const normalized = element.toLowerCase();
        const config = ELEMENT_CONFIG[normalized] || { badge: 'bg-gray-100 text-gray-600', icon: '' };
        return `<span class="badge-element ${config.badge} gap-1">${config.icon} ${t('element.' + normalized, element).toUpperCase()}</span>`;
    };
            ...
            <div class="flex items-start gap-4 pl-2 mb-auto">
                ...
                <div class="flex-grow min-w-0">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="text-lg font-bold text-slate-800 leading-tight truncate pr-2">${t(build.name, build.name) || t('dashboard.empty.title', 'Sem Nome')}</h3>
        ...
        const slotsHtml = ELEMENTS.map((element, index) => {
            ...
            if (gem && gem.attributes && gem.attributes.length > 0) {
                ...
                const attributesHtml = attributesToList.map(attr => {
                    ...
                    const aName = aObj ? t(aObj.name, aObj.name) : t(attr.name, attr.name) || 'Desconhecido';
                ...
                <div class="w-full text-left overflow-hidden">
                    ...
                    <div class="text-[10px] text-slate-400 truncate mt-1 font-medium border-t border-slate-100 pt-1">
                        ${t('editor.rarity', 'Raridade')}: ${gem.rarity || 'B'}
                    </div>
                </div>
            `;
                ...
                <div class="flex flex-col items-center justify-center h-16 text-slate-300 
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-widest">${t('editor.emptySlot', 'Vazio')}</span>
                </div>
            `;
            }
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold
                                    ...
                                    ${t('editor.rarityGem', 'Raridade (Gema)')}
                                </label>
                        ...
                        <div>
                            <div class="flex justify-between items-end
                                ...
                                <h4 class="font-bold text-slate-700">
                                    ${t('modal.gem.attributes', 'Atributos')}
                                </h4>
                                <span class="text-xs text-slate-400">
                                    ${t('modal.gem.maxAttr', 'Máx: 3')}
                                </span>
                            </div>
                            ...
                            <button type="button"
                                ...
                                ${t('modal.gem.addLine', '+ Adicionar Linha')}
                            </button>
                        </div>
                        ...
                        <div class="mt-8 flex gap-3 pt-6
                            ...
                            <button type="submit"
                                    class="flex-[2] btn-hover
                                           ...
                                           hover:bg-indigo-700">
                                ${t('modal.gem.save', 'Salvar Alterações')}
                            </button>
                        </div>
                    </form>
                </div>
    ...
    const renderMasterAttributesList = (attributes) => {
        ...
        if (!container) return;
        if (attributes.length === 0) { container.innerHTML = `<p class="text-slate-400 text-sm text-center py-4">${t('artifact.noAttr', 'Nenhum atributo cadastrado.')}</p>`; return; }
        ...
        const html = sorted.map(attr => `
            <div class="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-lg hover:border-indigo-200 transition-colors group mb-2">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-medium text-slate-700 text-sm">${t(attr.name, attr.name)}</span>
    ...
    const renderRequiredAttributesList = (requiredAttributes, masterAttributes) => {
        ...
        if (requiredAttributes.length === 0) {
            container.innerHTML = `<p class="text-slate-400 text-sm italic">${t('artifact.emptyList', 'Lista vazia.')}</p>`;
            return;
        }
            ...
            html += `
                <div class="flex justify-between items-center p-3 bg-white border border-green-100 rounded-lg mb-2 shadow-sm transition-all hover:shadow-md">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span class="font-bold text-slate-700 text-sm">${t(masterAttr.name, masterAttr.name)}</span>
    ...
    const renderSecondaryAttributesList = (secondaryAttributes, masterAttributes) => {
        ...
        if (!container) return;
        if (secondaryAttributes.length === 0) { container.innerHTML = `<p class="text-slate-400 text-sm italic">${t('artifact.emptyList', 'Lista vazia.')}</p>`; return; }
    ...
    const renderRecommendedCombosList = (combos, masterAttributes) => {
        ...
        if (!container) return;
        if (combos.length === 0) { container.innerHTML = `<p class="text-slate-400 text-sm italic">${t('artifact.noCombo', 'Nenhum combo.')}</p>`; return; }

        let html = '';
        combos.forEach(combo => {
            const attrNames = combo.attribute_ids.map(id => {
                const a = masterAttributes.find(m => m.id === id);
                return a ? t(a.name, a.name) : '?';
            }).join(', ');

            html += `
                <div class="p-4 border border-indigo-100 rounded-xl bg-indigo-50/50 mb-3 hover:bg-indigo-50 transition-colors">
                    <div class="flex justify-between items-start mb-2">
                        <h4 class="font-bold text-indigo-900">${t(combo.name, combo.name)} <span class="text-xs font-normal text-indigo-500 bg-white px-2 py-0.5 rounded border border-indigo-100 ml-2">${combo.rarity} +${combo.plus_level}</span></h4>
                        <div class="flex gap-2">
            ...
            <div id="admin-modal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                    <div class="p-6 border-b bg-indigo-50 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-indigo-900">${attr ? t('modal.masterAttr.edit', 'Editar') : t('modal.masterAttr.create', 'Criar')} ${t('modal.masterAttr.name', 'Atributo')}</h3>
                    ...
                    <form id="master-attribute-form" class="p-6 space-y-4">
                        ...
                        <div><label class="text-xs font-bold text-slate-500 uppercase">${t('modal.masterAttr.name', 'Nome')}</label><input type="text" id="master-attr-name" value="${attr?.name || ''}" class="w-full border-slate-300 rounded-lg text-sm mt-1"></div>
                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="text-xs font-bold text-slate-500 uppercase">${t('modal.masterAttr.tier', 'Tier')}</label><select id="master-attr-tier" class="w-full border-slate-300 rounded-lg text-sm mt-1">${tierOptions}</select></div>
                            <div><label class="text-xs font-bold text-slate-500 uppercase">${t('modal.masterAttr.element', 'Elemento')}</label><select id="master-attr-element" class="w-full border-slate-300 rounded-lg text-sm mt-1"><option value="">${t('misc.global', 'Global')}</option>${elementOptions}</select></div>
                        </div>
                        <button type="submit" class="w-full bg-indigo-600 text-white font-bold py-2.5 rounded-xl hover:bg-indigo-700 transition-colors">${t('modal.masterAttr.save', 'Salvar')}</button>
                    </form>
                </div>
            </div>`;
        document.getElementById('modals-container').innerHTML = modalHtml;
    ...
    const renderRequiredAttributeModal = (masterAttributes, editItem = null) => {
        ...
        const opts = masterAttributes.map(a => {
            ...
            return `<option value="${a.id}" ${selected}>${t(a.name, a.name)} (Lv${a.tier})</option>`;
        }).join('');
            ...
            <div id="admin-modal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-[fadeIn_0.2s_ease-out]">
                    <div class="p-6 border-b bg-green-50 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-green-900">${isEdit ? t('modal.required.edit', 'Editar Requisito') : t('modal.required.add', 'Adicionar Requisito')}</h3>
                    ...
                    <form id="required-attribute-form" class="p-6 space-y-4">
                        ...
                        <div>
                            <label class="text-xs font-bold text-slate-500 uppercase">${t('modal.required.attr', 'Atributo')}</label>
                        ...
                        <div class="flex items-center gap-3 p-3 bg-red-50 rounded-lg border border-red-100">
                            ...
                            <label for="required-attr-urgent" class="text-sm font-bold text-red-700 cursor-pointer select-none">
                                ${t('modal.required.urgent', 'Marcar como Prioridade/Urgente? 🔥')}
                            </label>
                        </div>
                        ...
                        <button type="submit" class="w-full bg-green-600 text-white font-bold py-2.5 rounded-xl hover:bg-green-700 transition-colors shadow-lg shadow-green-200">
                            ${isEdit ? t('modal.required.saveBtn', 'Salvar Alterações') : t('modal.required.addBtn', 'Adicionar')}
                        </button>
                    </form>
                </div>
            </div>`;
    ...
    const renderSecondaryAttributeModal = (masterAttributes) => {
        const opts = masterAttributes.map(a => `<option value="${a.id}">${t(a.name, a.name)} (Lv${a.tier})</option>`).join('');
        const modalHtml = `
            <div id="admin-modal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                    <div class="p-6 border-b bg-blue-50 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-blue-900">${t('modal.secondary.add', 'Adicionar Secundário')}</h3>
                        <button id="close-admin-modal-btn" class="text-slate-400 hover:text-slate-600">&times;</button>
                    </div>
                    <form id="secondary-attribute-form" class="p-6 space-y-4">
                        <div><label class="text-xs font-bold text-slate-500 uppercase">${t('modal.secondary.attr', 'Atributo')}</label><select id="secondary-attr-id" class="w-full border-slate-300 rounded-lg text-sm mt-1">${opts}</select></div>
                        <button type="submit" class="w-full bg-blue-600 text-white font-bold py-2.5 rounded-xl hover:bg-blue-700 transition-colors">${t('modal.secondary.addBtn', 'Adicionar')}</button>
                    </form>
                </div>
            </div>`;
        document.getElementById('modals-container').innerHTML = modalHtml;
    ...
    const renderRecommendedComboModal = (masterAttributes, combo = null) => {
        const opts = masterAttributes.map(a => `<option value="${a.id}" ${combo?.attribute_ids.includes(a.id) ? 'selected' : ''}>${t(a.name, a.name)}</option>`).join('');
        const modalHtml = `
    <div id="admin-modal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div class="p-6 border-b bg-indigo-50 flex justify-between items-center">
                <h3 class="text-lg font-bold text-indigo-900">
                    ${combo ? t('modal.combo.edit', 'Editar') : t('modal.combo.new', 'Novo')} Combo
                </h3>
                <button id="close-admin-modal-btn" class="text-slate-400 hover:text-slate-600">&times;</button>
            </div>
        ...
        const modalHtml = `
            <div id="modal-backdrop" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.2s_ease-out]">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden p-8 text-center border border-slate-200">
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-800 mb-2">${t('alert.login.restricted', 'Acesso Restrito')}</h3>
                    <p class="text-slate-500 text-sm mb-6">${t('alert.login.desc', 'Esta área é reservada para administradores.')}</p>
                    
                    <form id="login-form" class="space-y-4">
                        <div>
                            <input type="password" id="login-password" placeholder="${t('alert.login.placeholder', 'Digite a senha...')}" class="w-full text-center text-lg tracking-widest rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-3 transition-colors" autofocus>
                        </div>
                        <p id="login-error" class="text-red-500 text-xs font-bold hidden animate-bounce">${t('alert.login.wrong', 'Senha Incorreta!')}</p>
                        ...
                        <button type="submit" class="w-full btn-hover bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all">
                            ${t('alert.login.unlock', 'Desbloquear')}
                        </button>
                    </form>
                    <button id="cancel-login-btn" class="mt-4 text-slate-400 text-sm hover:text-slate-600 underline">${t('alert.login.cancel', 'Cancelar')}</button>
                </div>
            </div>
        `;
        document.getElementById('modals-container').innerHTML = modalHtml;
